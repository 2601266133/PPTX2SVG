 ...
 * 
 * 
 * if (idx >= 1 && idx <= 999) { childIdx = idx - 1; styleLst = (isLineStyle) ?
 * matrix.getLnStyleLst() : matrix.getFillStyleLst(); } else if (idx >= 1001) {
 * childIdx = idx - 1001; styleLst = matrix.getLnStyleLst(); } else { return
 * null; }
 * 
 * when >=1001 //getBgFillStyleLst() origin change to getLnStyleLst() to get
 * color or will get null for background color, it not totally solve the
 * background color not right but better --by jacsong2
 * 
 */
...
public abstract class XSLFShape implements Shape<XSLFShape, XSLFTextParagraph> {
	...
	public final XmlObject getXmlObject() {
		// it's final because the xslf inheritance hierarchy is not necessary the same
		// as
		// the (not existing) xmlbeans hierarchy and subclasses shouldn't narrow it's
		// return value
		return _shape;
	}
	...
	/**
	 * Returns a unique identifier for this shape within the current document. This
	 * ID may be used to assist in uniquely identifying this object so that it can
	 * be referred to by other parts of the document.
	 * <p>
	 * If multiple objects within the same document share the same id attribute
	 * value, then the document shall be considered non-conformant.
	 * </p>
	 *
	...
	/**
	 * Set the contents of this shape to be a copy of the source shape. This method
	 * is called recursively for each shape when merging slides
	 *
	 * @param sh
	 *            the source shape
	 ...
	 *            {@code "http://schemas.openxmlformats.org/presentationml/2006/main"}
	 * @param nodename
	...
	/**
	 * Specifies that the corresponding shape should be represented by the
	 * generating application as a placeholder. When a shape is considered a
	 * placeholder by the generating application it can have special properties to
	 * alert the user that they may enter content into the shape. Different types of
	 * placeholders are allowed and can be specified by using the placeholder type
	 * attribute for this element
	 *
	 * @param placeholder
	 *            The shape to use as placeholder or null if no placeholder should
	 *            be set.
	 */
	 ...
	 * properties. This method is the generalized form of selecting and casting
	 * those properties.
	 *
	 * @param resultClass
	 ...
	 *            context color
	 * @param parentPart
	 *            the parent package part. Any external references (images, etc.)
	 ...
	 */
	protected static PaintStyle selectPaint(XSLFFillProperties fp, final CTSchemeColor phClr,
			final PackagePart parentPart, final XSLFTheme theme, boolean hasPlaceholder) {
	...
	protected static PaintStyle selectPaint(CTSolidColorFillProperties solidFill, CTSchemeColor phClr,
	...
	protected static PaintStyle selectPaint(final CTGradientFillProperties gradFill, CTSchemeColor phClr,
			final XSLFTheme theme) {
	...
	protected static PaintStyle selectPaint(CTStyleMatrixReference fillRef, final XSLFTheme theme, boolean isLineStyle,
		...
		// background fill style within the presentation's style matrix, defined by the
		// fmtScheme element.
		// value of 0 or 1000 indicates no background,
		// values 1-999 refer to the index of a fill style within the fillStyleLst
		// element
		// values 1001 and above refer to the index of a background fill style within
		// the bgFillStyleLst element.
		...
		// see http://officeopenxml.com/prSlide-color.php - "Color Placeholders within
		// Themes"
		if (res != null || hasPlaceholder) {
			return res;
		}
		XSLFColor col = new XSLFColor(fillRef, theme, phClr);
		ColorStyle style = col.getColorStyle();

		return DrawPaint.createSolidPaint(style);
	}

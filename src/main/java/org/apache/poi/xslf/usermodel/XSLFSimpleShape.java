...
public abstract class XSLFSimpleShape extends XSLFShape implements SimpleShape<XSLFShape, XSLFTextParagraph> {
	...
	/**
	 * Get default line properties defined in the theme (if any). Used internally to
	...
	/**
	 * @param color
	 *            the color to paint the shape outline. A <code>null</code> value
	 *            turns off the shape outline.
	 */
	...
	/**
	 *
	 * @return the color of the shape outline or <code>null</code> if outline is
	 *         turned off
	 */
		...
		if (gp.isSetPrstGeom()) {
			...
			if (geom == null) {
				throw new IllegalStateException(
						"Unknown shape geometry: " + name + ", available geometries are: " + dict.keySet());
			}

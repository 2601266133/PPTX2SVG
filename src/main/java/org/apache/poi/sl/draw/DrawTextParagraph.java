...
public class DrawTextParagraph implements Drawable {
	...
	/**
	 * Defines an attribute used for storing the hyperlink associated with some
	 * renderable text.
	 */
	...
	/**
	 * break text into lines, each representing a line of text that fits in the
	 * wrapping width
	 *
	 * @param graphics
	...
	protected void breakText(Graphics2D graphics) {
		...
		for (;;) {
			...
			// shape width can be smaller that the sum of insets (this was proved by a test
			// file)
			if (wrappingWidth < 0) {
				wrappingWidth = 1;
			}
			...
			AttributedString str = (emptyParagraph) ? null // we will not paint empty paragraphs
	...
	protected double getWrappingWidth(boolean firstLine, Graphics2D graphics) {
		...
		if (!ts.getWordWrap()) {
			...
			// if wordWrap == false then we return the advance to the (right) border of the
			// sheet
			switch (textDir) {
			default:
	...
	private PlaceableShape<?, ?> getParagraphShape() {
		return new PlaceableShape() {
			...
			public Rectangle2D getAnchor() {
				return paragraph.getParentShape().getAnchor();
			}
			...
			public Sheet<?, ?> getSheet() {
				return paragraph.getParentShape().getSheet();
			}
		};
	}
	...
	protected AttributedString getAttributedString(Graphics2D graphics, StringBuilder text) {
		...
		for (TextRun run : paragraph) {
			...
			if (run.isBold()) {
				attList.add(new AttributedStringData(TextAttribute.WEIGHT, TextAttribute.WEIGHT_BOLD, beginIndex,
						endIndex));
			}
			if (run.isItalic()) {
				attList.add(new AttributedStringData(TextAttribute.POSTURE, TextAttribute.POSTURE_OBLIQUE, beginIndex,
						endIndex));
			}
			if (run.isUnderlined()) {
				attList.add(new AttributedStringData(TextAttribute.UNDERLINE, TextAttribute.UNDERLINE_ON, beginIndex,
						endIndex));
				attList.add(new AttributedStringData(TextAttribute.INPUT_METHOD_UNDERLINE,
						TextAttribute.UNDERLINE_LOW_TWO_PIXEL, beginIndex, endIndex));
			}
			if (run.isStrikethrough()) {
				attList.add(new AttributedStringData(TextAttribute.STRIKETHROUGH, TextAttribute.STRIKETHROUGH_ON,
						beginIndex, endIndex));
			}
			if (run.isSubscript()) {
				attList.add(new AttributedStringData(TextAttribute.SUPERSCRIPT, TextAttribute.SUPERSCRIPT_SUB,
						beginIndex, endIndex));
			}
			if (run.isSuperscript()) {
				attList.add(new AttributedStringData(TextAttribute.SUPERSCRIPT, TextAttribute.SUPERSCRIPT_SUPER,
						beginIndex, endIndex));
			}
	 ...
	 * <li>determine the font group - a text run can have different font groups.
	 * Depending on the chars, the correct font group needs to be used
	 *
	 * @param graphics
	 * @param dfm
	 * @param attList
	 ...
	 *
	 * @see <a href=
	 *      "https://blogs.msdn.microsoft.com/officeinteroperability/2013/04/22/office-open-xml-themes-schemes-and-fonts/">Office
	 *      Open XML Themes, Schemes, and Fonts</a>
	 */
	private void processGlyphs(Graphics2D graphics, DrawFontManager dfm, List<AttributedStringData> attList,
			final int beginIndex, TextRun run, String runText) {
		// determine font group ranges of the textrun to focus the fallback handling
		// only on that font group
		...
		for (FontGroupRange ttr : ttrList) {
			...
			while (partEnd < rangeBegin + rangeLen) {
				...
				if (partBegin < partEnd) {
					// handle (b) and (c)
					attList.add(new AttributedStringData(TextAttribute.FAMILY, fontMapped.getFontName(Locale.ROOT),
							beginIndex + partBegin, beginIndex + partEnd));
					if (LOG.check(POILogger.DEBUG)) {
						LOG.log(POILogger.DEBUG, "mapped: ", fontMapped.getFontName(Locale.ROOT), " ",
								(beginIndex + partBegin), " ", (beginIndex + partEnd), " - ",
								runText.substring(beginIndex + partBegin, beginIndex + partEnd));
					}
				}
				...
				if (partBegin < partEnd) {
					// handle (a) and (b)
					attList.add(new AttributedStringData(TextAttribute.FAMILY, fontFallback.getFontName(Locale.ROOT),
							beginIndex + partBegin, beginIndex + partEnd));
					if (LOG.check(POILogger.DEBUG)) {
						LOG.log(POILogger.DEBUG, "fallback: ", fontFallback.getFontName(Locale.ROOT), " ",
								(beginIndex + partBegin), " ", (beginIndex + partEnd), " - ",
								runText.substring(beginIndex + partBegin, beginIndex + partEnd));
					}
				}

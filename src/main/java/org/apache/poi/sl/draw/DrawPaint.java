 ...
 *
 * @see <a href="https://tips4java.wordpress.com/2009/07/05/hsl-color/">HSL code
 *      taken from Java Tips Weblog</a>
 */
public class DrawPaint {
	...
	private static class SimpleSolidPaint implements SolidPaint {
		...
		SimpleSolidPaint(final Color color) {
			...
			this.solidColor = new ColorStyle() {
				...
				@Override
				public int getAlpha() {
					return (int) Math.round(color.getAlpha() * 100000. / 255.);
				}
	 ...
	 *
	 * @see <a href=
	 *      "https://msdn.microsoft.com/en-us/library/dd560821%28v=office.12%29.aspx">Using
	 *      Office Open XML to Customize Document Formatting in the 2007 Office
	 *      System</a>
	 * @see <a href=
	 *      "https://social.msdn.microsoft.com/Forums/office/en-US/040e0a1f-dbfe-4ce5-826b-38b4b6f6d3f7/saturation-modulation-satmod">saturation
	 *      modulation (satMod)</a>
	 * @see <a href=
	 *      "http://stackoverflow.com/questions/6754127/office-open-xml-satmod-results-in-more-than-100-saturation">Office
	 *      Open XML satMod results in more than 100% saturation</a>
	 */
	public static Color applyColorTransform(ColorStyle color) {
		// TODO: The colors don't match 100% the results of Powerpoint, maybe because we
		// still
	 ...
	 *
	 * Example for lumMod/lumOff: The lumMod value is the percent luminance. A
	 * lumMod value of "60000", is 60% of the luminance of the original color. When
	 * the color is a shade of the original theme color, the lumMod attribute is the
	 * only one of the tags shown here that appears. The <a:lumOff> tag appears
	 * after the <a:lumMod> tag when the color is a tint of the original. The lumOff
	 * value always equals 1-lumMod, which is used in the tint calculation
	 *
	 * Despite having different ways to display the tint and shade percentages, all
	 * of the programs use the same method to calculate the resulting color. Convert
	 * the original RGB value to HSL ... and then adjust the luminance (L) with one
	 * of the following equations before converting the HSL value back to RGB. (The
	 * % tint in the following equations refers to the tint, themetint, themeshade,
	 * or lumMod values, as applicable.)
	 *
	 * @param hsl
	 *            the hsl values
	 * @param hslPart
	 *            the hsl part to modify [0..2]
	 * @param mod
	 *            the modulation adjustment
	 * @param off
	 ...
	 * Apply the tint
	 *
	 * For a tint, the equation is luminance * %tint + (1-%tint). (Note that 1-%tint
	 * is equal to the lumOff value in DrawingML.)
	 */
	...
	protected Paint createLinearGradientPaint(GradientPaint fill, Graphics2D graphics) {
		// TODO: we need to find the two points for gradient - the problem is, which
		// point at the outline
		// do you take? My solution would be to apply the gradient rotation to the shape
		// in reverse
		...
		AffineTransform at = AffineTransform.getRotateInstance(Math.toRadians(angle), anchor.getCenterX(),
				anchor.getCenterY());
		...
		if (p1.equals(p2)) {
			// gradient paint on the same point throws an exception ... and doesn't make
			// sense
			return null;
		}
	...
	public static int srgb2lin(float sRGB) {
		// scRGB has a linear gamma of 1.0, scale the AWT-Color which is in sRGB to
		// linear RGB

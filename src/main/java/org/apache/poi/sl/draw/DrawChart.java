...
/**
 * maybe in here, implements the function draw chart for XLSFChart
 * 
 **/

public class DrawChart extends DrawShape {

	public DrawChart(XSLFChartShape shape) {
		super(shape);
	}
		...
		if (fill != null) {
			for (Outline o : elems) {
				if (o.getPath().isFilled()) {
					Paint fillMod = drawPaint.getPaint(graphics, getShape().getFillStyle().getPaint(),
							o.getPath().getFill());
					if (fillMod != null) {
	...
	protected static CustomGeometry getCustomGeometry(String name, Graphics2D graphics) {
		...
		Map<String, CustomGeometry> presets = (graphics == null) ? null
		...
		if (presets == null) {
			...
			try {
				...
				while (staxFiltRd.peek() != null) {
					...
					JAXBElement<org.apache.poi.sl.draw.binding.CTCustomGeometry2D> el = unmarshaller
							.unmarshal(staxReader, CTCustomGeometry2D.class);
	...
	protected Collection<Outline> computeOutlines(Graphics2D graphics) {
		final XSLFChartShape sh = getShape();
	...
	@Override
	protected XSLFChartShape getShape() {
		return (XSLFChartShape) shape;
	}

 ...
 * method. Refer to the DOMTreeManager and DOMGroupManager documentation for
 * details on the structure of the generated DOM tree.
 *
 * The SVGGraphics2D class can produce a DOM tree using any implementation that
 * conforms to the W3C DOM 1.0 specification (see http://www.w3.org). At
 * construction time, the SVGGraphics2D must be given a org.w3.dom.Document
 * instance that is used as a factory to create the various nodes in the DOM
 * tree it generates.
 *
 * The various graphic context attributes (e.g., AffineTransform, Paint) are
 * managed by a GraphicContext object.
 *
 *
...
public class SVGGraphics2D extends AbstractGraphics2D implements Cloneable, SVGSyntax, ErrorConstants {
	 ...
	 * overridden by a child element is greater than DEFAULT_MAX_GC_OVERRIDES, a new
	 * group is created.
	 *
	 * @see org.apache.batik.svggen.DOMTreeManager
	 */
	 ...
	 * for this Graphics2D object. Once a group is complete, the group manager
	 * appends it to the DOM tree through the DOMTreeManager. Note that each
	 * SVGGraphics2D instance has its own DOMGroupManager (i.e., its own current
	 ...
	 * 
	 * @param svgCanvasSize
	 *            SVG Canvas size. May be null (equivalent to 100%, 100%)
	 */
	...
	/**
	 * @return the SVGShape used by this SVGGraphics2D instance to turn Java2D
	 *         shapes into SVG Shape objects.
	 */
	 ...
	 * 
	 * @param treeMgr
	 *            the new DOM Tree manager this SVGGraphics2D should use
	 */
	 ...
	 * 
	 * @param groupMgr
	 *            the new DOM Group manager this SVGGraphics2D should use
	 */
	...
	/**
	 * @return the Document used as a DOM object factory by this SVGGraphics2D
	 *         instance
	 */
	 ...
	 *            new extension handler this SVGGraphics2D should use
	 */
	...
	/**
	 * @param domFactory
	 *            Factory which will produce Elements for the DOM tree this
	 *            Graphics2D generates.
	 * @exception SVGGraphics2DRuntimeException
	 *                if domFactory is null.
	 */
	 ...
	 *            Graphics2D generates.
	 * @param imageHandler
	 *            defines how images are referenced in the generated SVG fragment
	 * @param extensionHandler
	 *            defines how Java 2D API extensions map to SVG Nodes.
	 * @param textAsShapes
	 *            if true, all text is turned into SVG shapes in the convertion. No
	 *            SVG text is output.
	 *
	 * @exception SVGGraphics2DRuntimeException
	 *                if domFactory is null.
	 */
	public SVGGraphics2D(Document domFactory, ImageHandler imageHandler, ExtensionHandler extensionHandler,
	 ...
	 */
	public static SVGGeneratorContext buildSVGGeneratorContext(Document domFactory, ImageHandler imageHandler,
	 ...
	 * 
	 * @param generatorCtx
	 *            the <code>SVGGeneratorContext</code> instance that will provide
	 *            all useful information to the generator.
	 * @param textAsShapes
	 *            if true, all text is turned into SVG shapes in the convertion. No
	 *            SVG text is output.
	 *
	...
	/**
	 * @param svgFileName
	 *            name of the file where SVG content should be written
	 */
	 ...
	 *            name of the file where SVG content should be written
	 * @param useCss
	 *            defines whether the output SVG should use CSS style properties as
	 *            opposed to plain attributes.
	 */
	...
	/**
	 * @param writer
	 *            used to writer out the SVG content
	 */
	 ...
	 * @param escaped
	 *            defines if the characters will be escaped style properties as
	 *            opposed to plain attributes.
	 */
	 ...
	 * @param escaped
	 *            defines if the characters will be escaped properties as opposed to
	 *            plain attributes.
	 */
	 ...
	 * converters. This also resets the converters.
	 */
	...
	/**
	 * Invoking this method will return a reference to the topLevelGroup Element
	 * managed by this object. It will also cause this object to start working with
	 * a new topLevelGroup.
	 *
	 ...
	 * a new topLevelGroup.
	 *
	 * @param includeDefinitionSet
	 *            if true, the definition set is included and the converters are
	 *            reset (i.e., they start with an empty set of definitions).
	 * @return top level group
	 */
	...
	/**
	 * Sets the topLevelGroup to the input element. This will throw an exception if
	 * the input element is not of type 'g' or if it is null.
	 */
	...
	/**
	 * @return the svg root node of the SVG document associated with this object
	 */
	...
	/**
	 * This version of the getRoot method will append the input svgRoot and set its
	 * attributes.
	 *
	 * @param svgRoot
	 *            an SVG element underwhich the content should be appended.
	 * @return the svg root node of the SVG document associated with this object.
	 */
	...
	public boolean drawImage(Image img, int x, int y, ImageObserver observer) {
		Element imageElement = getGenericImageHandler().createElement(getGeneratorContext());
		AffineTransform xform = getGenericImageHandler().handleImage(img, imageElement, x, y, img.getWidth(null),
				img.getHeight(null), getGeneratorContext());
	...
	public boolean drawImage(Image img, int x, int y, int width, int height, ImageObserver observer) {
		Element imageElement = getGenericImageHandler().createElement(getGeneratorContext());
		AffineTransform xform = getGenericImageHandler().handleImage(img, imageElement, x, y, width, height,
				getGeneratorContext());
	 ...
	 * @param x
	 *            the x coordinate in user space where the upper left corner of the
	 *            image is rendered
	 * @param y
	 *            the y coordinate in user space where the upper left corner of the
	 *            image is rendered
	 * @see #setTransform(AffineTransform)
	 * @see #setComposite(java.awt.Composite)
	 * @see #setClip(Shape)
	...
	public void drawImage(BufferedImage img, BufferedImageOp op, int x, int y) {
		...
		//
		/*
		 * if(img.getType() == BufferedImage.TYPE_INT_ARGB){ // // There are two special
		 * cases: AffineTransformOp and // ColorConvertOp. If the input op is not one of
		 * these // two, then SVGBufferedImageOp is requested to map the // filter to an
		 * SVG equivalent. // if(op instanceof AffineTransformOp){ AffineTransformOp
		 * transformOp = (AffineTransformOp)op; AffineTransform transform =
		 * transformOp.getTransform();
		 * 
		 * if(transform.getDeterminant() != 0){ AffineTransform inverseTransform = null;
		 * try{ inverseTransform = transform.createInverse();
		 * }catch(NoninvertibleTransformException e){ // This should never happen since
		 * we checked the // matrix determinant throw new
		 * SVGGraphics2DRuntimeException(ERR_UNEXPECTED); } gc.transform(transform);
		 * drawImage(img, x, y, null); gc.transform(inverseTransform); } else{
		 * AffineTransform savTransform = new AffineTransform(gc.getTransform());
		 * gc.transform(transform); drawImage(img, x, y, null);
		 * gc.setTransform(savTransform); } } else if(op instanceof ColorConvertOp){ img
		 * = op.filter(img, null); drawImage(img, x, y, null); } else{ // // Try and
		 * convert to an SVG filter // SVGFilterDescriptor filterDesc =
		 * domTreeManager.getFilterConverter().toSVG(op, null); if(filterDesc != null){
		 * // // Because other filters may be needed to represent the // composite that
		 * applies to this image, a group is created that // contains the image element.
		 * // Element imageElement = getDOMFactory(). createElementNS(SVG_NAMESPACE_URI,
		 * SVG_IMAGE_TAG); getImageHandler().handleImage((Image)img, imageElement,
		 * generatorCtx); imageElement.setAttributeNS(null, SVG_X_ATTRIBUTE,
		 * Integer.toString(x)); imageElement.setAttributeNS(null, SVG_Y_ATTRIBUTE,
		 * Integer.toString(y)); imageElement.setAttributeNS(null, SVG_WIDTH_ATTRIBUTE,
		 * Integer.toString(img.getWidth(null))); imageElement.setAttributeNS(null,
		 * SVG_HEIGHT_ATTRIBUTE, Integer.toString(img.getHeight(null)));
		 * imageElement.setAttributeNS(null, SVG_FILTER_ATTRIBUTE,
		 * filterDesc.getFilterValue()); Element imageGroup =
		 * generatorCtx.domFactory.createElementNS(SVG_NAMESPACE_URI, SVG_G_TAG);
		 * imageGroup.appendChild(imageElement);
		 * 
		 * domGroupManager.addElement(imageGroup); } else{ // // Could not convert to an
		 * equivalent SVG filter: // filter now and draw resulting image // img =
		 * op.filter(img, null); drawImage(img, x, y, null); } } } else{
		 */
		//
	...
	public void drawRenderedImage(RenderedImage img, AffineTransform trans2) {
		...
		Element image = getGenericImageHandler().createElement(getGeneratorContext());
		AffineTransform trans1 = getGenericImageHandler().handleImage(img, image, img.getMinX(), img.getMinY(),
				img.getWidth(), img.getHeight(), getGeneratorContext());
	 ...
	 * @param x
	 *            the x coordinate where the <code>String</code> should be rendered
	 * @param y
	 *            the y coordinate where the <code>String</code> should be rendered
	 * @see #setPaint(Paint)
	 * @see java.awt.Graphics#setColor
	...
	public void drawString(AttributedCharacterIterator ati, float x, float y) {
		...
		while (ch != AttributedCharacterIterator.DONE) {
			...
			Element tspan = text;
			if (multiSpans) {
				tspan = getDOMFactory().createElementNS(SVG_NAMESPACE_URI, SVG_TSPAN_TAG);
			...
			if (generatorCtx.isEmbeddedFontsOn()) {
				...
				// only create glyphs for the characters that are
				// needed
				getDOMTreeManager().getGraphicContextConverter().getFontConverter().recordFontUsage(s, getFont());
			}
			...
			domTreeManager.getStyleHandler().setStyle(tspan, deltaGC.getContext(),
	...
	/**
	 * Set the Element attributes, depending on the AttributedCharacterIterator
	 * attributes. The following attributes are set :
	 * <ul>
	 * <li>underline</li>
	...
	private void setTextElementFill(AttributedCharacterIterator ati) {
		if (ati.getAttribute(TextAttribute.FOREGROUND) != null) {
			if (ati.getAttribute(TextAttribute.FOREGROUND) instanceof Color) {
				Color color = (Color) ati.getAttribute(TextAttribute.FOREGROUND);
				setPaint(color);
			} else if (ati.getAttribute(TextAttribute.FOREGROUND) instanceof LinearGradientPaint) {
				Paint paint = (Paint) ati.getAttribute(TextAttribute.FOREGROUND);
				setPaint(paint);
			}
		}
	}
	...
	private void setTextElementAttributes(SVGGraphicContext tspanGC, AttributedCharacterIterator ati) {
		...
		if (len != 0)
			tspanGC.getContext().put(CSS_TEXT_DECORATION_PROPERTY, decoration.substring(0, len - 1));
	}

	/**
	 * Return true if the AttributedCharacterIterator is bold (at its current
	 * position).
	 */
	...
	/**
	 * Return true if the AttributedCharacterIterator is italic (at its current
	 * position).
	 */
	private boolean isItalic(AttributedCharacterIterator ati) {
		Object attr = ati.getAttribute(TextAttribute.POSTURE);
		if (TextAttribute.POSTURE_OBLIQUE.equals(attr))
			return true;
		return false;
	}

	/**
	 * Return true if the AttributedCharacterIterator is underlined (at its current
	 * position).
	 */
	private boolean isUnderline(AttributedCharacterIterator ati) {
		Object attr = ati.getAttribute(TextAttribute.UNDERLINE);
		if (TextAttribute.UNDERLINE_ON.equals(attr))
			return true;
	...
	/*
	 * This is the list of attributes that can't currently be supported by
	 * drawString(AttributedCharacterIterator). For accuracy if any of these are
	 * present then the text is drawn as outlines.
	 */
	...
	/**
	 * This method let's users indicate that they don't care that certain text
	 * attributes will not be properly converted to SVG, in exchange when those
	 * attributes are used they will get real SVG text instead of paths.
	 *
	 * @param attrs
	 *            The set of attrs to treat as unsupported, and if present cause
	 *            text to be drawn as paths. If null ACI text will be rendered as
	 *            text (unless 'textAsShapes' is set).
	 */
	public void setUnsupportedAttributes(Set attrs) {
		if (attrs == null)
			unsupportedAttributes = null;
		else
			unsupportedAttributes = new HashSet(attrs);
	}
